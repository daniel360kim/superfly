...
class PegasusApp:
    ...
    def _spawn_lighting(self):
        ...
        lives in superfly.sim.scene_setup, shared with scripts/scene_audit.py
        so the vetting audit predicts exactly what this harness does)."""
        scene_setup.spawn_lighting(headlamp_parent="/World/quadrotor")

    def _add_colliders(self, root: str = "/World/layout", approximation: str = "none",
                       verbose: bool = True):
        """Static triangle-mesh colliders for a loaded USD stage (body lives in
        superfly.sim.scene_setup, shared with scripts/scene_audit.py -- incl.
        the load-bearing de-instancing fix for UE-exported stages)."""
        return scene_setup.add_colliders(self.world.stage, root=root,
                                         approximation=approximation, verbose=verbose)
